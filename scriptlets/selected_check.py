...
class SelectedCheck(Scriptlet):
    ...
    def _modes_required(self, **kwargs):
        return 3;

    def _check_mode(self, **kwargs):
        del kwargs

        modes_required = self._modes_required()
        ...
        if player.vars.get("mode_pick") == 7 and mode_complete_count >= modes_required:
            self.machine.events.post('selected_mode_is_available')
    ...
    def _check_wiz(self, **kwargs):
        ...
        if mode_complete_count < modes_required:
